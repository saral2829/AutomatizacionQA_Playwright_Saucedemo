...
public class CartStepsDef {
    ...
    public CartStepsDef(CartSteps cart) {
        this.cart = cart;
    }

    @Given("el usuario inicia sesión como {string} y {string}")
    public void el_usuario_inicia_sesión_como_y(String user, String pass) {
        cart.login(user, pass);
    }

    @Given("estoy en la página de productos")
    public void estoy_en_la_página_de_productos() {
        cart.assertProductsPage();
    }

    @Given("el carrito está vacío")
    public void el_carrito_está_vacío() {
    ...
    public void el_carrito_debe_mostrar_artículos(Integer expected) {
        cart.assertCartItems(expected);
    }

    @Then("los nombres en el carrito deben ser {string}")
    public void los_nombres_en_el_carrito_deben_ser(String csv) {
        cart.assertCartNames(csv);
    }
}
