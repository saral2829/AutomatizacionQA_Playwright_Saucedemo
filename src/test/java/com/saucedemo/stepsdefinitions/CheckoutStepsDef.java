...
public class CheckoutStepsDef {
    ...
    public void el_usuario_está_logueado_como_con_password(String user, String pass) {
        cart.login(user, pass);
    }

    @Given("el usuario agregó {int} productos al carrito")
    public void el_usuario_agregó_productos_al_carrito(Integer n) {
        cart.addNDefaultProducts(n);
    }
    ...
    public void hace_click_en(String buttonText) { co.clickCheckout(); }
    ...
    public void veo_mensaje_confirmacion(String msg) {
        co.assertCompleteMessage(msg);
    }

    // Negativos
    @Then("veo el mensaje de error {string}")
    public void veo_mensaje_de_error(String msg) {
        co.assertFormError(msg);
    }
}
